...
- **Closed-world `-p:X=Y` is supported.** Sorted `(Key=Value)` pairs are folded into the shape hash. Cache miss → codegen runs with `ProjectCollection.GlobalProperties` → `project.GlobalProperties` are unconditionally re-applied in `InitialState.Populate` (overriding csproj defaults). Same project + same `-p:` set → cache hit. Non-`TargetFramework` global-property sets use isolated `.bsharp/variants/<hash>/` cache roots; `TargetFramework` variants live under that root's `inner/<tfm>/` directory.
...
- **Cross-project contract direction:** v1 ProjectReference support uses serial referenced-project execution through constrained local `AssignProjectConfiguration`, `ResolveProjectReferences`, and `<MSBuild>` task paths. Supported calls start with static project lists and `Build`/`GetTargetPath`-style targets that can return item identity/metadata to the consuming host; dynamic project lists and unsupported target/property sets fail with explicit diagnostics. The launcher pre-restores static `ProjectReference` graphs with `dotnet restore` and then runs the generated host with `--no-restore`; SDK restore graph recursion remains outside the generated v1 slice. Generated hosts try referenced bsharp hosts first when launched by bsharp and fall back to `dotnet build --no-restore` if the referenced generated host cannot yet build that shape. The default ProjectReference E2E covers cold/warm build, referenced-host generation, app execution, referenced-source incremental rebuild/copy-local, and referenced-project shape invalidation; the gated SDK and normalized non-SDK dependency corpus cases are promoted under the same contract.
