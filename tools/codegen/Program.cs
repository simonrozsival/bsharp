...
if (args.Contains("--emit-go-replay", StringComparer.OrdinalIgnoreCase)) {
    ...
    var goOutDir = ArgValue(args, "--out-dir") ?? ArgValue(args, "--out");
    var goSdkFp = ArgValue(args, "--sdk-fingerprint") ?? "";
    if (goTrace == null || goOutDir == null) {
        Console.Error.WriteLine("usage: codegen --emit-go-replay --trace <jsonl> --out-dir <dir> [--sdk-fingerprint <fp>]");
        return 1;
    }
    return Codegen.EmitGoReplay(goTrace, goOutDir, goSdkFp);
}
...
static class Codegen {
    ...
    public static int EmitGoReplay(string tracePath, string outDir, string sdkFingerprint) {
        if (!File.Exists(tracePath)) {
            Console.Error.WriteLine($"codegen: trace file not found: {tracePath}");
            return 2;
        }
        Directory.CreateDirectory(outDir);

        // Parse JSONL trace and emit one Go literal entry per call. The literal
        // contains the pre-serialized request bytes so the Go binary never has
        // to re-marshal anything: it just writes the length-prefixed frame and
        // reads the response back.
        var lines = File.ReadAllLines(tracePath).Where(l => l.Length > 0).ToList();
        var entries = new List<(string TaskName, string TargetName, byte[] ReqBytes)>(lines.Count);
        foreach (var line in lines) {
            using var doc = JsonDocument.Parse(line);
            var req = doc.RootElement.GetProperty("req");
            var taskName = req.TryGetProperty("TaskName", out var tn) ? tn.GetString() ?? "?" : "?";
            var targetName = req.TryGetProperty("TargetName", out var tg) ? tg.GetString() ?? "" : "";
            // Re-serialize the req element on its own so the embedded bytes are
            // exactly what the daemon expects (no JSONL wrapping).
            var reqBytes = Encoding.UTF8.GetBytes(req.GetRawText());
            entries.Add((taskName, targetName, reqBytes));
        }

        var goMod = "module bsharpgen\n\ngo 1.21\n";
        File.WriteAllText(Path.Combine(outDir, "go.mod"), goMod);

        // Emit a minimal Go program that opens a Unix socket to bsharp-taskd,
        // handshakes, then replays each embedded frame in order.
        var sb = new StringBuilder(64 * 1024 + entries.Sum(e => e.ReqBytes.Length * 5));
        sb.AppendLine("// Code generated by bsharp/codegen --emit-go-replay. DO NOT EDIT.");
        ...
        sb.AppendLine(")");
        sb.AppendLine();
        sb.AppendLine($"const sdkFingerprint = {QuoteGoString(sdkFingerprint)}");
        sb.AppendLine("const daemonVersion = \"1\"");
        sb.AppendLine("const protocolVersion = 2");
        sb.AppendLine();
        sb.AppendLine("// Embedded task invocations captured at codegen time.");
        sb.AppendLine("// Each entry is a fully serialized TaskInvocation JSON payload.");
        sb.AppendLine("var calls = []struct {");
        sb.AppendLine("\tname   string");
        sb.AppendLine("\ttarget string");
        sb.AppendLine("\treq    []byte");
        sb.AppendLine("}{");
        foreach (var e in entries) {
            sb.Append("\t{");
            sb.Append(QuoteGoString(e.TaskName));
            sb.Append(", ");
            sb.Append(QuoteGoString(e.TargetName));
            sb.Append(", []byte(");
            AppendGoStringLiteral(sb, e.ReqBytes);
            sb.AppendLine(")},");
        }
        sb.AppendLine("}");
        sb.AppendLine();
        sb.Append(GoReplayMainSource);
        File.WriteAllText(Path.Combine(outDir, "main.go"), sb.ToString());

        Console.WriteLine($"Wrote {outDir}/main.go ({entries.Count} task calls, {sb.Length} bytes)");
        Console.WriteLine($"Build with: cd {outDir} && go build -ldflags '-linkmode=external' -o bsharp-replay && codesign -s - --force bsharp-replay");
        return 0;
    }

    static string QuoteGoString(string s) {
        ...
        foreach (var c in s) {
            ...
            else if (c < 0x20) b.Append($"\\x{(int)c:x2}");
            else b.Append(c);
        }
        b.Append('"');
        return b.ToString();
    ...
    static void AppendGoStringLiteral(StringBuilder sb, byte[] bytes) {
        sb.Append('"');
        foreach (var b in bytes) {
            ...
            else if (b < 0x20 || b >= 0x7f) sb.Append($"\\x{b:x2}");
            else sb.Append((char)b);
        }
        sb.Append('"');
    }

    const string GoReplayMainSource = """"
        ...
        func main() {
        	...
        	for _, a := range os.Args[1:] {
        		switch {
        		case a == "build" || a == "run" || a == "audit":
        			cmd = a
        		case a == "-v" || a == "-v:detailed" || a == "-v:normal":
        			verbose = true
        		case strings.HasPrefix(a, "-v"):
        			// quiet/minimal: ignore
        		case a == "--no-restore" || a == "--no-fast-noop":
        			// recognized, ignored at replay
        		}
        	}
        	switch cmd {
        	case "audit":
        		fmt.Println("audit: no-op (generated Go replay host)")
        		return
        	case "run":
        		fmt.Fprintln(os.Stderr, "run: not implemented in Go replay host")
        		os.Exit(1)
        	}
        	...
        	if err != nil {
        		if daemonExe := os.Getenv("BSHARP_TASKD_PATH"); daemonExe != "" {
        			if err := spawnDaemon(daemonExe, sock); err != nil {
        				fatal("spawn daemon: %v", err)
        			}
        			conn, err = dialWithRetry(sock, 5*time.Second)
        		}
        		if err != nil {
        			fatal("connect to daemon at %s: %v", sock, err)
        		}
        	}
        	defer conn.Close()
        	...
        	if err := writeFrame(conn, hsBytes); err != nil {
        	...
        	_ = json.Unmarshal(resp, &hr)
        	if hr.Error != "" {
        		fatal("handshake refused: %s", hr.Error)
        	}

        	// Replay.
        	for i, c := range calls {
        		if verbose {
        			fmt.Fprintf(os.Stderr, "[%d] %s (target=%s)\n", i, c.name, c.target)
        		}
        		if err := writeFrame(conn, c.req); err != nil {
        			fatal("call %d (%s): write: %v", i, c.name, err)
        		}
        		respBytes, err := readFrame(conn)
        		if err != nil {
        			fatal("call %d (%s): read: %v", i, c.name, err)
        		}
        		var tr struct {
        			Success bool   `json:"Success"`
        			Error   string `json:"Error"`
        		}
        		_ = json.Unmarshal(respBytes, &tr)
        		if !tr.Success {
        			fatal("call %d (%s) failed: %s", i, c.name, tr.Error)
        		}
        	}
        }
        ...
        func socketPath() string {
        	...
        	return filepath.Join(dir, fmt.Sprintf("taskd-%s-%s.sock", daemonVersion, sdkFingerprint))
        }
        ...
        func spawnDaemon(daemonExe, sock string) error {
        	logPath := strings.TrimSuffix(sock, ".sock") + ".log"
        	shell := fmt.Sprintf(`exec %s --sdk-fingerprint %s --socket %s </dev/null >>%s 2>>%s &`,
        ...
        func fatal(format string, args ...any) {
        	fmt.Fprintf(os.Stderr, "bsharp-replay: "+format+"\n", args...)
        	os.Exit(1)
        }
        """";
...
if (!noBuild) {
    ...
    if (!fastNoOp) {
        ...
        if (!string.Equals(P.Get("UseMaui"), "true", StringComparison.OrdinalIgnoreCase) &&
    ...
    static void EmitNativeAotTaskRoots(StringBuilder sb, TaskMetadataLoader.MetadataIndex? meta) {
        ...
        if (meta != null) {
            foreach (var t in meta.ByTaskName.Values.OrderBy(t => t.FullTypeName, StringComparer.Ordinal)) {
                var assemblyName = Path.GetFileNameWithoutExtension(t.AssemblyPath);
                sb.AppendLine($"    [DynamicDependency(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.PublicProperties, {CSharpLiteral(t.FullTypeName)}, {CSharpLiteral(assemblyName)})]");
            }
        }
        sb.AppendLine("    [DynamicDependency(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.PublicProperties, \"Microsoft.CSharp.CSharpCodeProvider\", \"System.CodeDom\")]");
        sb.AppendLine("    [DynamicDependency(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.NonPublicMethods, \"Microsoft.Build.Utilities.ToolTask\", \"Microsoft.Build.Utilities.Core\")]");
        sb.AppendLine("    [DynamicDependency(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.NonPublicMethods, \"Microsoft.CodeAnalysis.BuildTasks.ManagedToolTask\", \"Microsoft.Build.Tasks.CodeAnalysis\")]");
